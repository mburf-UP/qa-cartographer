<!-- Produced by the cartographer in cold run 02 on 2026-08-22, by a Claude session with no memory
     of the build and no prior sight of this territory. Reproduced byte-for-byte from the transcript
     in runs/02-class-A-front-door.md so it can be tested as an artifact in its own right.
     Not improved and not corrected - its errors are its own. The ONLY changes are the
     de-identification substitutions declared in the note below, which touch names and never
     claims. -->
